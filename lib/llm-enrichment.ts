...
export async function extractWithLLM(
    ...
    try {
        ...
        // Validate salary range — toss obviously wrong values rather than letting
        // them through. Same band as the prompt text (config/niche/salary.ts) so
        // the two enforcement points can never drift.
        if (parsed.salary_min && (parsed.salary_min < salaryConfig.enrichment.annualMin || parsed.salary_min > salaryConfig.enrichment.annualMax)) {
