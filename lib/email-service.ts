...
// Fallbacks read config/brand.ts (the single source of truth) so an
// unset env can never send as another board's domain; runtime values
// come from env (validated in lib/env.ts).
...
// from the creator, not an automated job-board notification. Falls back
// to EMAIL_FROM_MARKETING if unset (the previous behavior).
...
export async function sendRefundConfirmationEmail(
  ...
  try {
    ...
    const html = emailShellV2(
      ...
      ${spacerV2(28)}
      <tr><td class="content-pad" style="padding:0 40px;text-align:center;"><p style="margin:0;font-family:${SANS_V2};font-size:14px;color:${V2.textMuted};line-height:1.6;">Questions? Reply to this email or contact <a href="mailto:${brand.email.support}" style="color:${V2.teal};text-decoration:underline;">${brand.email.support}</a>.</p></td></tr>
      ${spacerV2(48)}
      ${closeContentV2()}`,
      unsubscribeFooterV2(unsubscribeToken || 'sample'),
...
/**
 * Delivery email for the salary-guide lead magnet.
 *
 * The body must describe the PDF that is actually attached behind
 * `pdfUrl` — the artifact generated by scripts/generate-salary-pdf.ts.
 * That guide deliberately prints NO state-by-state wage table and no
 * remote-vs-in-person differential (the disclosed-salary pool mixes
 * annualized locum hourly rates and physician postings with W-2
 * salaries, so freezing it into a permanent document would overstate
 * typical pay — see the note above fetchLiveInventory in the generator).
 * The copy below mirrors the PDF's own cover lede.
 *
 * Overpromising here is exactly the P0 #6 failure mode in reverse: the
 * link resolves, but the description does not. Do not re-add a claim
 * without adding the corresponding section to the PDF first;
 * tests/regressions/p1-salary-pdf-copy-truth.test.ts guards this string.
 */
export function buildSalaryGuideHtml(pdfUrl: string, unsubscribeToken: string): string {
  return emailShellV2(`
    ${headerBlockV2(`Your ${SALARY_GUIDE_EDITION_YEAR} Salary Guide`, '')}
    ${spacerV2(12)}
    ${simpleBlock('hero-salary-guide.png', `Your ${brand.niche.short} compensation report is ready. It covers the federal national wage benchmark, pay by experience level, practice setting and specialty premium, and what to check before you negotiate. State-by-state pay changes daily, so the guide points you to the free live table on the site rather than freezing a snapshot.`)}
    ${spacerV2(32)}
    <tr><td class="content-pad" style="padding:0 40px;text-align:center;">
      ${primaryButtonV2('Download Salary Guide (PDF)', pdfUrl)}
    </td></tr>
    ${spacerV2(16)}
    <tr><td class="content-pad" style="padding:0 40px;text-align:center;"><p style="margin:0;font-family:${SANS_V2};font-size:14px;color:${V2.textMuted};line-height:1.6;">Want pay by state? See the <a href="${BASE_URL}/salary-guide" style="color:${V2.teal};text-decoration:underline;">daily-updated state table</a>. Looking for opportunities? <a href="${BASE_URL}/jobs" style="color:${V2.teal};text-decoration:underline;">Browse open positions</a>.</p></td></tr>
    ${spacerV2(48)}
    ${closeContentV2()}`,
    unsubscribeFooterV2(unsubscribeToken),
