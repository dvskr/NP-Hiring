...
// Seller identity on a legal financial document — must come from brand
// config, never a hardcoded default (a fork would otherwise issue
// invoices under the wrong legal entity).
const DEFAULT_FROM_NAME = brand.legal.brandDisplayName;
const DEFAULT_FROM_ADDRESS: ReadonlyArray<string> = [
  brand.legal.addressLine,
  `${brand.legal.addressCity}, ${brand.legal.governingState} ${brand.legal.addressPostalCode}`,
  'United States',
];
const DEFAULT_FROM_EMAIL = brand.email.support;
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.footer}>
          <Text>Thank you for using {brand.legal.brandDisplayName}.</Text>
          <Text style={{ marginTop: 4 }}>Questions? {brand.email.support}</Text>
          <Text style={{ marginTop: 8, fontSize: 8, color: '#9CA3AF' }}>
            {brand.legal.entityName} · {brand.legal.address}
          </Text>
        </View>
      </Page>
    </Document>
