...
export function validateAndNormalizeSalary(
  ...
  // invalid to clamp-on-invalid per user request.) Bounds live in
  // config/niche/salary.ts and MUST be retuned per niche — wrong bounds
  // fabricate salaries, they don't just filter them.
  // Widened to a string-indexed Record at the use site (the config object
  // stays deep-readonly); unknown periods fall through clampToBounds's
