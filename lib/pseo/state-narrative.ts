...
const SETTING_LEADS: Record<string, SettingLeadFn> = {
    ...
    'addiction': (c) => `Addiction-focused ${brand.niche.short} roles in ${c.stateName} require a DEA X-waiver for buprenorphine prescribing and frequently involve medication-assisted treatment (MAT) programs. NHSC loan repayment is broadly available for substance-use treatment positions.`,
    ...
    'behavioral-health': (c) => `Integrated behavioral health ${brand.niche.short} roles in ${c.stateName} embed ${brand.niche.adjective} NPs inside primary-care teams, FQHCs, and school-based clinics — typically 8–12 brief consults per day rather than full 30–60 minute sessions. Many sites qualify for NHSC loan repayment under value-based contracts that prioritize population mental-health outcomes.`,
};
