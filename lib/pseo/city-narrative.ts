...
const TAXONOMY_LEADS: Record<string, TaxonomyLeadFn> = {
    'remote': (f) => `Remote ${brand.niche.short} positions covering patients in ${f.city.stateCode} typically pay $110K–$170K and require active state licensure plus a HIPAA-compliant home setup. Many roles offer DEA-registered prescribing across multiple states via the Nurse Licensure Compact.`,
    ...
    'locum-tenens': (f) => `Locum tenens ${brand.niche.short} coverage in ${f.city.name} pays $80–$150 per hour with malpractice and travel costs covered by the agency. Assignments range from 4–26 weeks and are common for filling permanent-hire gaps.`,
    ...
    'per-diem': (f) => `Per-diem ${brand.niche.short} shifts in ${f.city.name} pay $60–$110 per hour without scheduled commitment. Most providers stack per-diem coverage with a primary employer for income smoothing.`,
