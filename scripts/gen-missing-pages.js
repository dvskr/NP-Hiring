...
// Read inpatient as template
const template = fs.readFileSync('app/jobs/inpatient/page.tsx', 'utf-8');

const pages = [
  { slug: 'community-health', title: 'Community Health', filter: 'community', q: 'community+health', salary: '$120K-170K', heroBg: '#b8d4c8' },
  { slug: 'contract', title: 'Contract', filter: 'contract', q: 'contract', salary: '$130K-180K', heroBg: '#c8d6e5' },
  { slug: 'crisis', title: 'Crisis', filter: 'crisis', q: 'crisis', salary: '$140K-200K', heroBg: '#e5c8c8' },
  { slug: 'entry-level', title: 'Entry Level', filter: 'entry level', q: 'entry+level', salary: '$100K-140K', heroBg: '#d6e5c8' },
  { slug: 'full-time', title: 'Full-Time', filter: 'full-time', q: 'full+time', salary: '$130K-180K', heroBg: '#c8d6e5' },
  { slug: 'geriatric', title: 'Geriatric', filter: 'geriatric', q: 'geriatric', salary: '$130K-175K', heroBg: '#d6c8e5' },
  { slug: 'hospital', title: 'Hospital', filter: 'hospital', q: 'hospital', salary: '$140K-200K', heroBg: '#b8d4e3' },
  { slug: 'lgbtq', title: 'LGBTQ+', filter: 'LGBTQ', q: 'LGBTQ', salary: '$120K-170K', heroBg: '#e5d6c8' },
  { slug: 'mid-career', title: 'Mid-Career', filter: 'mid-career', q: 'mid+career', salary: '$140K-190K', heroBg: '#c8e5d6' },
  { slug: 'part-time', title: 'Part-Time', filter: 'part-time', q: 'part+time', salary: '$60K-100K', heroBg: '#e5e5c8' },
  { slug: 'private-practice', title: 'Private Practice', filter: 'private practice', q: 'private+practice', salary: '$150K-250K', heroBg: '#d4c8b8' },
  { slug: 'senior', title: 'Senior', filter: 'senior', q: 'senior+PMHNP', salary: '$160K-220K', heroBg: '#c8d4e5' },
  { slug: 'veterans', title: 'Veterans', filter: 'veteran', q: 'veteran', salary: '$130K-180K', heroBg: '#b8c8d4' },
];

function makePage(p) {
  const funcName = p.slug.replace(/-([a-z])/g, (_, c) => c.toUpperCase()) + 'Page';
  const UpperName = funcName.charAt(0).toUpperCase() + funcName.slice(1);
...
const WHERE_CLAUSE = {
  isPublished: true,
  OR: [
    { title: { contains: '${p.filter}', mode: 'insensitive' as const } },
  ],
};
...
const ${p.slug.replace(/-/g,'')}Faqs = [
  { question: 'What is a ${p.title} PMHNP role?', answer: '${p.title} PMHNP positions focus on psychiatric mental health care in ${p.title.toLowerCase()} settings, offering specialized clinical opportunities.' },
  { question: 'What does a ${p.title} PMHNP earn?', answer: '${p.title} PMHNPs typically earn ${p.salary} annually depending on location, experience, and setting.' },
  ...
  { question: 'How do I find ${p.title.toLowerCase()} positions?', answer: 'Browse our curated ${p.title.toLowerCase()} job listings above, set up alerts, and apply directly through our platform.' },
];

export async function generateMetadata(): Promise<Metadata> {
  const stats = await getStats();
  return {
    title: \`\${stats.totalJobs} ${p.title} PMHNP Jobs (${p.salary})\`,
    description: \`Find \${stats.totalJobs} ${p.title.toLowerCase()} PMHNP jobs paying ${p.salary}+. Browse psychiatric nurse practitioner positions.\`,
    alternates: { canonical: 'https://pmhnphiring.com/jobs/${p.slug}' },
  };
}

interface PageProps { searchParams: Promise<{ page?: string }>; }

export default async function ${UpperName}({ searchParams }: PageProps) {
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      <BreadcrumbSchema items={[
        ...
        { name: "${p.title}", url: "https://pmhnphiring.com/jobs/${p.slug}" }
      ]} />
      <JobListViewTracker jobs={jobs.map((j: Job) => ({ id: j.id, title: j.title, employer: j.employer }))} listName="${p.title} Jobs" />

      {/* ═══ HERO ═══ */}
      <section style={{ background: '${p.heroBg}', padding: '72px 0 56px' }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          <div className="cat-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', alignItems: 'center' }}>
            <div>
              <p style={{ fontSize: '13px', fontWeight: 700, color: '#831843', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '12px' }}>{stats.totalJobs}+ Open Positions</p>
              <h1 className="font-lora" style={{ fontSize: 'clamp(32px, 4.2vw, 48px)', fontWeight: 800, lineHeight: 1.08, color: '#1A2E35', margin: '0 0 20px' }}>${p.title}<br /><span style={{ color: '#BE185D' }}>PMHNP Jobs</span></h1>
              <p style={{ fontSize: '16px', color: '#3D2E26', lineHeight: 1.7, margin: '0 0 36px', maxWidth: '440px' }}>Browse ${p.title.toLowerCase()} psychiatric nurse practitioner positions with competitive pay and benefits.</p>
              <Link href="/jobs?q=${p.q}" className="cat-cta-primary" style={{ padding: '16px 40px', borderRadius: '16px', fontWeight: 700, fontSize: '15px', background: '#BE185D', color: '#fff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '10px', boxShadow: '4px 4px 14px rgba(190,24,93,0.25)' }}>Browse ${p.title} Jobs <ArrowRight size={17} /></Link>
            </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 20px' }}>
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3">
            <h2 className="font-lora mb-6" style={{ fontSize: '20px', fontWeight: 700, color: '#1A2E35' }}>${p.title} Positions ({stats.totalJobs})</h2>
            {jobs.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', marginTop: '32px' }}>
              <Link href="/jobs?q=${p.q}" className="cat-cta-primary" style={{ padding: '14px 32px', borderRadius: '14px', fontWeight: 700, fontSize: '14px', background: '#BE185D', color: '#fff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '4px 4px 12px rgba(190,24,93,0.2)' }}>Browse All ${p.title} Jobs <ArrowRight size={16} /></Link>
            </div>
          </div>
          <div className="lg:col-span-1">
            <div style={{ ...clayCard, padding: '24px', marginBottom: '20px', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)', border: '2px solid rgba(190,24,93,0.15)' }}>
              ...
              <h3 className="font-lora" style={{ fontSize: '18px', fontWeight: 700, color: '#831843', margin: '0 0 8px' }}>${p.title} Alerts</h3>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDF2F8 0%, #FDF2F8 50%, #FDF2F8 100%)' }}>
        <section style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 20px 40px' }}>
          <p style={{ fontSize: '13px', fontWeight: 600, color: '#E86C2C', textTransform: 'uppercase', letterSpacing: '0.15em', textAlign: 'center', marginBottom: '8px' }}>Why Choose ${p.title}</p>
          <h2 className="font-lora" style={{ fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: 700, color: '#1A2E35', textAlign: 'center', marginBottom: '48px' }}>Built for ${p.title} Care</h2>
          <div className="cat-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              ...
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>Focus on ${p.title.toLowerCase()} psychiatric care with dedicated clinical environments.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Competitive Pay</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>${p.title} PMHNPs earn ${p.salary} with benefits, loan repayment, and bonuses.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Growing Demand</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>High demand for ${p.title.toLowerCase()} PMHNPs nationwide with strong job security.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Career Growth</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>Advance from clinical roles to leadership positions in ${p.title.toLowerCase()} settings.</p>
            </div>
          </div>
        </section>
      </div>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDFBF7 0%, #FFF8F0 50%, #FDFBF7 100%)' }}>
        <section style={{ maxWidth: '1000px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <h2 className="font-lora" style={{ fontSize: 'clamp(24px, 3.2vw, 34px)', fontWeight: 700, color: '#1A2E35', textAlign: 'center', marginBottom: '40px' }}>${p.title} PMHNP Questions</h2>
          <div style={{ display: 'grid', gap: '16px' }}>
            {${p.slug.replace(/-/g,'')}Faqs.map((faq, idx) => (
...
for (const p of pages) {
  ...
  console.log(`${p.slug}: ✅ created (${makePage(p).split('\n').length} lines)`);
}
