...
const SLUG = 'mid-career';
const LABEL = 'Mid-Career';
/** Role noun for titles and band heads, e.g. "Mid-Career NP". */
const NOUN = labelNoun(SLUG, LABEL);
/** Mid-sentence label, e.g. "mid-career". */
...
const MC_FILTER = categoryWhere(SLUG);
...
function getFacts(): Promise<ListingFacts> {
  return getListingFacts(`category-landing:${SLUG}`, MC_FILTER);
}

async function getJobs(skip = 0, take = 10) {
  return prisma.job.findMany({ where: canonicalBucketWhere(MC_FILTER), orderBy: BEST_SORT_ORDER_BY, skip, take });
}
...
const EXPLORE_CARDS: readonly ExploreCard[] = [
  { href: '/jobs/private-practice', label: 'Private Practice', sub: 'Independent roles' },
  { href: '/jobs/locum-tenens', label: 'Locum Tenens', sub: 'Travel assignments' },
  { href: '/jobs/senior', label: 'Senior', sub: 'Executive roles' },
...
/**
 * LAND-L2 places. Mid-career has no /jobs/mid-career/[state] spoke, so
 * each state links to its hub, which renders for any state with a job.
 */
 ...
 *
 * P5: the precepting answer states the truthful mechanics (compensation
 * varies by employer, ask in negotiation) and may not regress to a
 * prevalence claim about preceptor pay.
 */
function buildFaqs(facts: ListingFacts) {
  const payAnswer = buildPostedPaySentence({ slug: SLUG, facts }) ?? NATIONAL_MEDIAN_SENTENCE;
  return [
    { q: `What defines a mid-career ${brand.niche.short} role?`, a: `Mid-career listings are the ones employers describe as needing established clinical practice rather than an entry-level start. That wording varies between employers, so read the experience, autonomy and supervision each listing states instead of relying on the label.` },
    ...
    { q: 'How do I transition into a specialty?', a: `Employers hiring into a specialty look for related clinical experience and the certification the specialty requires. Read the certification each listing names, check it against your own board certification, and ask what onboarding the employer provides.` },
    ...
    { q: `What do mid-career ${brand.niche.short}s earn?`, a: payAnswer },
  ];
}
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    title: buildCategoryLandingTitle({ role: NOUN, totalJobs, tagline: 'Leadership and Specialty' }),
...
export default async function MidCareerPage({ searchParams }: PageProps) {
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      <CategoryHero
        bgColor="#accfb9"
        heroImage="/images/categories/heroes/mid-career.webp"
        heroAlt={`Mid-career ${brand.niche.short} clinical leadership`}
        badgeText={facts.total > 0 ? buildLiveRolesBadge(facts.total) : `${brand.niche.long} Careers`}
        breadcrumbs={crumbsFromSchema([{ name: "Home", url: brand.baseUrl }, { name: "Jobs", url: `${brand.baseUrl}/jobs` }, { name: "Mid-Career", url: `${brand.baseUrl}/jobs/mid-career` }])}
        ...
        description={`Mid-career ${brand.niche.short} positions with leadership responsibility and specialty tracks.`}
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px' }}>
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <div style={{ ...clayCard, padding: '24px', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)', border: '2px solid rgba(190,24,93,0.15)' }}><Bell size={28} style={{ color: '#BE185D', marginBottom: '12px' }} /><h3 className="font-lora" style={{ fontSize: '18px', fontWeight: 700, color: '#831843', margin: '0 0 8px' }}>Mid-Career Alerts</h3><p style={{ fontSize: '13px', color: '#BE185D', marginBottom: '16px' }}>New mid-career roles delivered daily.</p><Link href="/job-alerts" style={{ display: 'block', textAlign: 'center', padding: '10px 20px', borderRadius: '10px', fontWeight: 700, fontSize: '13px', background: '#BE185D', color: '#fff', textDecoration: 'none' }}>Create Alert</Link></div>
          </div>
        </div>
      </div>
      ...
      {snapshotRenders && (
        <Band id="snapshot-mid-career" eyebrow="Market Snapshot" title={`What current ${NOUN} listings show`} background="#FDFBF7">
      ...
      {locationCards > 0 && (
        <Band id="locations-mid-career" eyebrow="Locations" title={`Where ${NOUN} listings are`} background={MINT_STAGE}>
        ...
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 20px 40px' }}>
          ...
          <div className="cat-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            <div className="cat-bento-hero-1" style={{ ...clayCard, gridColumn: 'span 8', padding: '0', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
              ...
              <ImmersiveImage src="/images/categories/bento/mid-career-lead.webp" alt="Leadership office" minHeight={240} />
            </div>
            <div className="cat-bento-hero-2" style={{ ...clayCard, gridColumn: 'span 4', padding: '0', overflow: 'hidden', display: 'flex', flexDirection: 'column', textAlign: 'center' }}>
              <ImmersiveImage src="/images/categories/bento/mid-career-specialize.webp" alt="Specialization" minHeight={200} />
            ...
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/mid-career-leader.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Leadership</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Supervisory and program director positions.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/mid-career-salary.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Posted Pay</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Each listing states its own pay; compare before you apply.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/mid-career-niche.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Specialization</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Deepen expertise in a focused clinical specialty.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/mid-career-teach.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Teaching</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Precept students and mentor new graduates.</p></div>
          </div>
        </section>
      </div>
      ...
      {payRenders && (
        <Band id="pay-mid-career" eyebrow="Compensation" title={`${NOUN} posted pay`} background="#FDFBF7">
          <div className="pseo-clay-split" style={{ gap: '14px', alignItems: 'stretch' }}>
            ...
            <div className="pseo-clay-card" style={{ ...clayCard, padding: 0, overflow: 'hidden', display: 'grid' }}>
              <ImmersiveImage src="/images/categories/bento/mid-career-salary.webp" alt="Mid-career pay" minHeight={240} />
            </div>
          </div>
        </Band>
      )}
      ...
      {axisGuide && (
        <Band id="guide-mid-career" eyebrow="Using This Page" title="How to use this page" background={PEACH_STAGE}>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDFBF7 0%, #FFF8F0 50%, #FDFBF7 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {[{ n: '01', t: `${brand.niche.short} Certification`, d: 'Active national certification in your specialty.' }, { n: '02', t: 'State License', d: 'APRN licensure and prescriptive authority.' }, { n: '03', t: 'Clinical Experience', d: 'Read the years of practice each listing asks for.' }, { n: '04', t: 'Leadership Skills', d: 'Supervisory, mentoring, or program management experience.' }].map(item => (
