...
const PD_FILTER = buildCategoryWhereClause('per-diem');

async function getJobs(skip = 0, take = 20) { return prisma.job.findMany({ where: PD_FILTER, orderBy: BEST_SORT_ORDER_BY, skip, take }); }
async function getStats() {
  const totalJobs = await prisma.job.count({ where: PD_FILTER });
  ...
  const medianSalaryK = await getGatedMedianKForWhere({ ...PD_FILTER, normalizedMinSalary: { not: null }, normalizedMaxSalary: { not: null } });
  const topEmployers = await prisma.job.groupBy({ by: ['employer'], where: PD_FILTER, _count: { employer: true }, orderBy: { _count: { employer: 'desc' } }, take: 8 });
...
const faqs = [
  { q: `What is a per diem ${brand.niche.short}?`, a: `A per diem ${brand.niche.short} works on an as-needed basis, picking up shifts at healthcare facilities without a fixed schedule or long-term commitment. Per diem means "per day": you are paid for each day you work, typically at higher hourly rates than permanent staff.` },
  { q: `How much do per diem ${brand.niche.short}s earn?`, a: `Per diem rates typically run higher per hour than comparable benefited staff pay, since no benefits are attached, and premium shifts (nights, weekends, holidays) usually carry differentials. Actual rates vary widely by specialty, setting, and region, and annual income depends on how many shifts you pick up.` },
  { q: 'What is the difference between per diem and PRN?', a: 'Per diem and PRN (pro re nata, meaning "as needed") are often used interchangeably. Both refer to on-call, shift-based work without guaranteed hours. Some facilities distinguish between them: per diem may carry minimum shift requirements, while PRN is truly as-needed.' },
  { q: `Do per diem ${brand.niche.short}s get benefits?`, a: 'Most per diem positions do not include benefits such as health insurance or PTO, which is why hourly rates are higher. Some hospital systems offer limited benefits (a 401(k) plan, employee discounts) to per diem staff who work a minimum number of shifts per month.' },
  { q: 'Can I work per diem at multiple facilities?', a: `Yes. Many per diem ${brand.niche.short}s are credentialed at 2 to 4 facilities simultaneously, which maximizes shift availability and income flexibility. Check non-compete clauses and confirm that each facility provides or accepts your malpractice coverage.` },
];
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Find ${stats.totalJobs} per diem ${brand.niche.short} positions. Pick your shifts, earn premium hourly rates, and avoid long-term commitments. Updated daily.`,
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      <CategoryHero
        ...
        breadcrumbs={crumbsFromSchema([{ name: "Home", url: brand.baseUrl }, { name: "Jobs", url: `${brand.baseUrl}/jobs` }, { name: "Per Diem", url: `${brand.baseUrl}/jobs/per-diem` }])}
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px' }}>
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3">
            <h2 className="font-lora mb-6" style={{ fontSize: '20px', fontWeight: 700, color: '#1A2E35' }}>Per Diem Positions ({stats.totalJobs})</h2>
            ...
            <div style={{ textAlign: 'center', marginTop: '32px' }}><Link href="/jobs?category=per-diem" className="cat-cta-primary" style={{ padding: '14px 32px', borderRadius: '14px', fontWeight: 700, fontSize: '14px', background: '#BE185D', color: '#fff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '4px 4px 12px rgba(190,24,93,0.2)' }}>Browse All Per Diem Jobs <ArrowRight size={16} /></Link></div>
          </div>
          <div className="lg:col-span-1">
            <div style={{ ...clayCard, padding: '24px', marginBottom: '20px', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)', border: '2px solid rgba(190,24,93,0.15)' }}><Bell size={28} style={{ color: '#BE185D', marginBottom: '12px' }} /><h3 className="font-lora" style={{ fontSize: '18px', fontWeight: 700, color: '#831843', margin: '0 0 8px' }}>Per Diem Alerts</h3><p style={{ fontSize: '13px', color: '#BE185D', marginBottom: '16px' }}>New shift-based positions posted daily.</p><Link href="/job-alerts" style={{ display: 'block', textAlign: 'center', padding: '10px 20px', borderRadius: '10px', fontWeight: 700, fontSize: '13px', background: '#BE185D', color: '#fff', textDecoration: 'none' }}>Create Alert</Link></div>
        ...
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 20px 40px' }}>
          ...
          <div className="cat-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            <div className="cat-bento-hero-1" style={{ ...clayCard, gridColumn: 'span 8', padding: '32px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', alignItems: 'center' }}>
              <div><h3 className="font-lora" style={{ fontSize: '20px', fontWeight: 700, color: '#1A2E35', margin: '0 0 10px' }}>Pick Your Shifts</h3><p style={{ fontSize: '14px', color: '#5A4A42', lineHeight: 1.7, margin: 0 }}>Choose which days you work, with no minimum hours or long-term commitment required. Get credentialed at multiple facilities to maximize your options.</p></div>
              <Image src={`${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/bento_pd_shifts.webp`} alt="Shift selection board" width={280} height={200} style={{ width: '100%', height: 'auto', borderRadius: '14px' }} />
            </div>
            <div className="cat-bento-hero-2" style={{ ...clayCard, gridColumn: 'span 4', padding: '28px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
              <Image src={`${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/bento_pd_free.webp`} alt="Flexibility" width={200} height={140} style={{ width: '100%', maxWidth: '180px', height: 'auto', borderRadius: '12px', marginBottom: '16px' }} />
            ...
            <div className="cat-bento-cta" style={{ ...clayCard, gridColumn: 'span 4', padding: '28px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)' }}>
              <Bell size={32} style={{ color: '#BE185D', marginBottom: '14px' }} /><h3 className="font-lora" style={{ fontSize: '18px', fontWeight: 700, color: '#831843', margin: '0 0 10px' }}>Get Per Diem Alerts</h3><p style={{ fontSize: '13px', color: '#BE185D', lineHeight: 1.6, margin: '0 0 20px' }}>New shift positions daily.</p>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDFBF7 0%, #FFF8F0 50%, #FDFBF7 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {[{ n: '01', t: 'Availability', d: 'Set clear shift preferences and minimum notice requirements.' }, { n: '02', t: 'Credentialing', d: 'Get credentialed at multiple facilities simultaneously.' }, { n: '03', t: 'Malpractice', d: 'Verify coverage. Some facilities provide it, while others require your own.' }, { n: '04', t: 'Tax Planning', d: 'Track income and expenses, as you may receive both W-2 and 1099 forms.' }].map(item => (
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDF2F8 0%, #FDF2F8 50%, #FDF2F8 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div className="cat-explore-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
            {[{ href: '/jobs/part-time', label: 'Part-Time', sub: 'Flexible hours', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_parttime.webp` }, { href: '/jobs/contract', label: 'Contract', sub: 'Fixed-term roles', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_contract.webp` }, { href: '/jobs/locum-tenens', label: 'Locum Tenens', sub: 'Travel assignments', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_locumtenens.webp` }, { href: '/jobs/full-time', label: 'Full-Time', sub: 'Standard schedules', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_fulltime.webp` }, { href: '/salary-guide', label: 'Salary Guide', sub: '2026 data', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_salary.webp` }, { href: '/jobs/locations', label: 'By Location', sub: '50 states', icon: `${STORAGE_BASE}/storage/v1/object/public/site-assets/images/categories/clay_icon_location.webp` }].map(c => (
