...
// Force dynamic rendering: not statically generated during build
...
const SLUG = 'remote';
const LABEL = 'Remote';
/** Role noun for titles and headings, e.g. "Remote NP". */
...
async function getRemoteJobs(skip: number = 0, take: number = 10) {
...
const EXPLORE_CARDS: ReadonlyArray<{ href: string; label: string; sub: string; icon: string }> = [
  ...
  { href: '/jobs/travel', label: 'Travel', sub: 'Assignment work', icon: '/images/categories/nav/travel.webp' },
  { href: '/jobs/new-grad', label: 'New Grad', sub: 'Entry-level roles', icon: '/images/categories/nav/new-grad.webp' },
  { href: '/jobs/per-diem', label: 'Per Diem', sub: 'Shift by shift', icon: '/images/categories/nav/per-diem.webp' },
...
export default async function RemoteJobsPage({ searchParams }: PageProps) {
  ...
  const jobs = facts.total > 0 ? await getRemoteJobs(skip, limit) : [];
  ...
  const breadcrumbTrail = [
    ...
    { name: "Remote", url: `${brand.baseUrl}/jobs/remote` },
  ];
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      {jobs.length > 0 && (
        <script
          ...
          dangerouslySetInnerHTML={{
            ...
            // through the repo's \u003c escape chain: an unescaped </script>
            // in a title would otherwise break out of the block.
      ...
      <CategoryHero
        ...
        description="Telehealth and remote positions you can hold from a home office, with the licensure states named in each listing."
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px' }}>
          <div className="grid lg:grid-cols-4 gap-8">
              ...
              {jobs.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                  {jobs.map((job: Job, i: number) => (
                    <JobCard key={job.id} job={job} listName={LIST_NAME} listIndex={skip + i} />
                  ))}
                </div>
              )}
            ...
            <div className="lg:col-span-1">
              <div className="remote-bento-card" style={{ ...clayCard, padding: '0', overflow: 'hidden', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)', border: '2px solid rgba(190,24,93,0.15)' }}>
                <div style={{ padding: '24px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#BE185D', marginBottom: '16px', lineHeight: 1.6, fontWeight: 500 }}>
                    New {MID} roles delivered to your inbox daily.
                  </p>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDF2F8 0%, #FDF2F8 50%, #FDF2F8 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 20px 40px' }}>
          ...
          <p style={{ fontSize: '15px', color: '#5A4A42', textAlign: 'center', maxWidth: '480px', margin: '0 auto 48px', lineHeight: 1.6 }}>
            Remote roles trade the commute for a home office, and each listing names the states it needs you licensed in.
          </p>
          ...
          <div className="remote-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            ...
            <div className="remote-bento-hero-2 remote-bento-card" style={{ ...clayCard, gridColumn: 'span 4', padding: '0', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '24px 22px', flex: 1 }}>
                ...
                <p style={{ fontSize: '12.5px', color: '#7A6A62', margin: 0, lineHeight: 1.5 }}>
                  Care for patients in more than one state. Some employers sponsor and pay for additional state licenses; the listing usually says so.
                </p>
              </div>
            </div>
            ...
            <div className="remote-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Listings state the hours they cover: mornings, evenings, or weekends.</p>
            </div>
            <div className="remote-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}>
              ...
              <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Virtual Care Work</h3>
              <p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Video visits, utilization review, and case management all appear in this pool.</p>
            </div>
            <div className="remote-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Employers supply the encrypted platform and the EHR access a remote role needs.</p>
            </div>
            <div className="remote-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Some listings describe supervision, onboarding, and mentorship for a first role.</p>
            </div>
          </div>
        </section>
      </div>
        ...
        <Band id={`pay-${SLUG}`} eyebrow="Compensation" title={`What ${NOUN} listings post`} background="#FDFBF7">
          <div className="pseo-clay-split" style={{ gap: '14px', alignItems: 'stretch' }}>
            ...
            <div className="pseo-clay-card" style={{ ...clayCard, padding: 0, overflow: 'hidden', display: 'grid' }}>
              <ImmersiveImage src="/images/categories/bento/remote-salary-growth.webp" alt={`Remote ${brand.niche.short} pay illustration`} minHeight={240} />
            </div>
          </div>
        </Band>
      )}
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDF2F8 0%, #E6FFFA 50%, #FDF2F8 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {[
              { step: '01', title: 'Platform Setup', text: 'Employers name the video and EHR platform in the listing, and most provide onboarding for it.' },
              { step: '02', title: 'Licensure', text: 'Plan for an active license in every state where your patients are located, and confirm the requirement with each board.' },
              { step: '03', title: 'Equipment', text: 'A reliable broadband connection, a quality webcam, a noise cancelling headset, and a private workspace.' },
              { step: '04', title: 'Workspace', text: 'A quiet, well-lit room with a neutral background and a door you can close while you see patients.' },
            ].map(r => (
      ...
      <style>{`
        ...
        .remote-cta-primary:hover { transform: translateY(-3px); box-shadow: 0 10px 32px rgba(190,24,93,0.35) !important; filter: brightness(1.05); }
        .remote-cta-secondary { transition: transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease; }
        .remote-cta-secondary:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.08) !important; border-color: rgba(190,24,93,0.3) !important; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .remote-cta-primary, .remote-cta-secondary, .remote-bento-card, .remote-stat-pill { transition: none; }
          .remote-cta-primary:hover, .remote-cta-secondary:hover, .remote-bento-card:hover, .remote-stat-pill:hover { transform: none; }
        }
      `}</style>
    </div>
  );
