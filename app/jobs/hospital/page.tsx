...
/** Role noun for titles and band heads, e.g. "Hospital NP". */
const NOUN = labelNoun(SLUG, LABEL);
/** Mid-sentence label, e.g. "hospital". */
...
function getFacts(): Promise<ListingFacts> {
  return getListingFacts(`category-landing:${SLUG}`, HO_FILTER);
}
...
const EXPLORE_CARDS: readonly ExploreCard[] = [
  { href: '/jobs/inpatient', label: 'Inpatient', sub: 'Hospital units' },
  { href: '/jobs/outpatient', label: 'Outpatient', sub: 'Clinic-based' },
  { href: '/jobs/emergency', label: 'Emergency', sub: 'ED and urgent care' },
  { href: '/jobs/geriatric', label: 'Geriatric', sub: 'Elder care' },
...
/**
 * LAND-L2 places. Hospital has no /jobs/hospital/[state] spoke, so each
 * state links to its hub, which renders for any state with a job.
 */
...
function buildFaqs(facts: ListingFacts) {
  ...
  return [
    ...
    { q: `What do hospital ${brand.niche.short}s earn?`, a: payAnswer },
    { q: `What schedule do hospital ${brand.niche.short}s work?`, a: 'Schedules are set by the unit and the employer. Read the schedule each listing states, including shift length, night and weekend rotation, and any on-call requirement, before you apply.' },
    { q: 'What qualifications are required?', a: `Hospital listings state their own requirements. Expect an active APRN license, national ${brand.niche.short} certification, DEA registration and current life support certification, and read each posting for the acute care experience it asks for. Some hospitals run residency and fellowship programs for newly certified ${brand.niche.short}s.` },
    { q: 'Do hospital systems offer loan forgiveness?', a: 'Eligibility depends on the employer and on the federal program rules in force when you apply. Ask each employer whether the site qualifies, and confirm the current requirements with the federal student aid office before you rely on them.' },
  ];
}
...
export default async function HospitalPage({ searchParams }: PageProps) {
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      <CategoryHero
        bgColor="#a0c3d6"
        heroImage="/images/categories/heroes/hospital.webp"
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px' }}>
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <div style={{ ...clayCard, padding: '24px', background: 'linear-gradient(145deg, #FDF2F8, #FCE7F3)', border: '2px solid rgba(190,24,93,0.15)' }}><Bell size={28} style={{ color: '#BE185D', marginBottom: '12px' }} /><h3 className="font-lora" style={{ fontSize: '18px', fontWeight: 700, color: '#831843', margin: '0 0 8px' }}>Hospital Alerts</h3><p style={{ fontSize: '13px', color: '#BE185D', marginBottom: '16px' }}>New hospital roles delivered daily.</p><Link href="/job-alerts" style={{ display: 'block', textAlign: 'center', padding: '10px 20px', borderRadius: '10px', fontWeight: 700, fontSize: '13px', background: '#BE185D', color: '#fff', textDecoration: 'none' }}>Create Alert</Link></div>
          </div>
        </div>
      </div>
      ...
      {snapshotRenders && (
        <Band id="snapshot-hospital" eyebrow="Market Snapshot" title={`What current ${NOUN} listings show`} background="#FDFBF7">
      ...
      {locationCards > 0 && (
        <Band id="locations-hospital" eyebrow="Locations" title={`Where ${NOUN} listings are`} background={MINT_STAGE}>
        ...
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 20px 40px' }}>
          ...
          <div className="cat-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            ...
            <div className="cat-bento-hero-1" style={{ ...clayCard, gridColumn: 'span 8', padding: '0', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
              <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}><h3 className="font-lora" style={{ fontSize: '20px', fontWeight: 700, color: '#1A2E35', margin: '0 0 10px' }}>Acute Inpatient Care</h3><p style={{ fontSize: '14px', color: '#5A4A42', lineHeight: 1.7, margin: 0 }}>Manage acute admissions, inpatient stabilization, and ER consultations in fast-paced hospital environments.</p></div>
              <ImmersiveImage src="/images/categories/bento/hospital-acute.webp" alt="Acute inpatient hospital unit" minHeight={240} />
            </div>
            <div className="cat-bento-hero-2" style={{ ...clayCard, gridColumn: 'span 4', padding: '0', overflow: 'hidden', display: 'flex', flexDirection: 'column', textAlign: 'center' }}>
              <ImmersiveImage src="/images/categories/bento/hospital-team.webp" alt="Hospital care team" minHeight={200} />
            ...
            {/* ROW 2: Icon Cards */}
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/hospital-acute.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Acute Care</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Manage acute admissions and inpatient stabilization.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/hospital-pay.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Posted Pay</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Each listing states its own pay, shift and differential terms.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/hospital-team.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Team Support</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Collaborate with physicians, RNs, and social workers.</p></div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 3', padding: '24px 18px', textAlign: 'center' }}><Image src="/images/categories/icons/hospital-loan.webp" alt="" width={48} height={48} style={{ width: '48px', height: '48px', objectFit: 'contain', margin: '0 auto 14px', display: 'block' }} /><h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1A2E35', margin: '0 0 6px' }}>Loan Forgiveness</h3><p style={{ fontSize: '12px', color: '#7A6A62', margin: 0, lineHeight: 1.55 }}>Ask whether the employer qualifies for federal forgiveness programs.</p></div>
          </div>
        </section>
      </div>
      ...
      {payRenders && (
        <Band id="pay-hospital" eyebrow="Compensation" title={`${NOUN} posted pay`} background="#FDFBF7">
          <div className="pseo-clay-split" style={{ gap: '14px', alignItems: 'stretch' }}>
            ...
            <div className="pseo-clay-card" style={{ ...clayCard, padding: 0, overflow: 'hidden', display: 'grid' }}>
              <ImmersiveImage src="/images/categories/bento/hospital-salary.webp" alt="Hospital pay" minHeight={240} />
            </div>
          </div>
        </Band>
      )}
      ...
      {axisGuide && (
        <Band id="guide-hospital" eyebrow="Using This Page" title="How to use this page" background={PEACH_STAGE}>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FDFBF7 0%, #FFF8F0 50%, #FDFBF7 100%)' }}>
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {[{ n: '01', t: `${brand.niche.short} Certification`, d: 'Active national certification in your specialty.' }, { n: '02', t: 'State License', d: 'APRN licensure and prescriptive authority.' }, { n: '03', t: 'DEA and Life Support', d: 'DEA registration and current basic or advanced life support.' }, { n: '04', t: 'Inpatient Experience', d: 'Read the acute care experience each listing asks for.' }].map(item => (
