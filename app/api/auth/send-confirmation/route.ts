...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Never log the action_link itself — it is a live one-click auth
    // credential; anyone with log access could hijack the account before
    // the user clicks it.
    ...
    const html = emailShellV2(`
      ...
      <tr><td class="content-pad" style="padding:0 40px;">
        <p style="margin:0 0 24px;font-family:${SERIF};font-size:19px;color:${V2.textBody};line-height:1.6;">
          Thanks for signing up for ${brand.name}! Please confirm your email address by clicking the button below.
        </p>
      </td></tr>
