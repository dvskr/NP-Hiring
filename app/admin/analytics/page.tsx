...
const card: React.CSSProperties = {
    backgroundColor: '#FAFBF9', border: '1px solid rgba(255,255,255,0.7)', borderRadius: '18px', boxShadow: '8px 8px 20px rgba(0,0,0,0.05), -6px -6px 16px rgba(255,255,255,0.9), inset 3px 3px 6px rgba(255,255,255,0.7), inset -2px -2px 4px rgba(0,0,0,0.02)', overflow: 'hidden',
};
...
const th: React.CSSProperties = {
    ...
    backgroundColor: '#F8FAF9', whiteSpace: 'nowrap',
};
const td: React.CSSProperties = {
    padding: '14px 16px', fontSize: '13px', color: '#6B7F8A',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: `${height}px`, paddingTop: '8px' }}>
            {data.map((d, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    <span style={{ fontSize: '10px', fontWeight: 600, color: '#6B7F8A' }}>{d.count}</span>
                    <div
                    ...
                    />
                    <span style={{ fontSize: '9px', color: '#94A3B8', transform: 'rotate(-45deg)', whiteSpace: 'nowrap' }}>
                        {d.date.slice(5)}
                    </span>
                </div>
            ))}
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <div style={{
                ...
                {icon}
                <span style={{ fontSize: '22px', fontWeight: 800, color: '#1A2E35' }}>{value.toLocaleString()}</span>
            </div>
            <div style={{ fontSize: '13px', fontWeight: 600, color: '#1A2E35' }}>{label}</div>
            {rate !== undefined && (
...
export default function AnalyticsPage() {
    ...
    return (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 16px' }}>
            ...
            {/* Tabs */}
            <div style={{ display: 'flex', borderBottom: '1px solid #E8ECF0', marginBottom: 24, overflowX: 'auto' }}>
                {([
                        ...
                        <div style={{ ...card, padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                                <div style={{ padding: '14px 16px', borderRadius: 10, backgroundColor: '#F8FAF9' }}>
                                    <div style={{ fontSize: 24, fontWeight: 800, color: '#1A2E35' }}>{s.activeJobs.toLocaleString()}</div>
                                    <div style={muted}>Active Jobs</div>
                                </div>
                                <div style={{ padding: '14px 16px', borderRadius: 10, backgroundColor: '#F8FAF9' }}>
                                    <div style={{ fontSize: 24, fontWeight: 800, color: '#1A2E35' }}>{s.employerPostedJobs}</div>
                                    <div style={muted}>Employer-Posted</div>
                                </div>
                            </div>
                            <div style={{ fontSize: 13, fontWeight: 600, color: '#6B7F8A', marginBottom: 8 }}>Jobs by Source</div>
                        ...
                        <div style={{ ...card, padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
                                {[
                                    ...
                                    <div key={r.label} style={{ padding: '12px', borderRadius: 10, backgroundColor: '#F8FAF9', textAlign: 'center' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                                <div style={{ padding: '14px 16px', borderRadius: 10, backgroundColor: '#F8FAF9' }}>
                                    ...
                                    <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>{s.totalSubscribers} total leads</div>
                                </div>
                                <div style={{ padding: '14px 16px', borderRadius: 10, backgroundColor: '#F8FAF9' }}>
                                    <div style={{ fontSize: 20, fontWeight: 800, color: '#22C55E' }}>{s.activeAlerts}</div>
                                    <div style={muted}>Active Alerts</div>
                                    <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>{s.dailyAlerts} daily · {s.weeklyAlerts} weekly</div>
                                </div>
                            </div>
                            {/* Employer leads */}
                            {s.totalEmployerLeads > 0 && (
                                <div style={{ marginTop: 12, padding: '12px 16px', borderRadius: 10, backgroundColor: '#F8FAF9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                    ...
                                    <div style={{ display: 'flex', gap: 12 }}>
                                        {Object.entries(s.employerLeadStatuses || {}).map(([status, count]) => (
                                            <div key={status} style={{ textAlign: 'center' }}>
                                                <div style={{ fontSize: 14, fontWeight: 700, color: '#1A2E35' }}>{count}</div>
                                                <div style={{ fontSize: 10, color: '#94A3B8', textTransform: 'capitalize' }}>{status}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4" style={{ marginBottom: 24 }}>
                        {[
                            ...
                            <div key={c.label} style={{ ...card, padding: '20px 22px' }}>
                                ...
                                <div style={{ fontSize: 28, fontWeight: 800, color: '#1A2E35' }}>{c.value.toLocaleString()}</div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    ...
                                    <tbody>
                                        {data.topJobs.map((job, i) => (
                                            <tr key={job.id}>
                                                <td style={td}>
                                                    <span style={{ fontWeight: 700, color: i < 3 ? ['#F59E0B', '#94A3B8', '#CD7F32'][i] : '#94A3B8' }}>
                                                        #{i + 1}
                                                    </span>
                                                </td>
                                                <td style={{ ...td, fontWeight: 600, color: '#1A2E35', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                    {job.title}
                                                </td>
                            ...
                            <div key={r.id} style={{
                                ...
                                {(r.reporterName || r.reporterEmail) && (
                                    <div style={{ fontSize: 12, color: '#3B82F6', marginBottom: 4 }}>
                                        ...
                                        {r.reporterEmail && <span style={{ color: '#94A3B8', marginLeft: 4 }}>({r.reporterEmail})</span>}
                                    </div>
                                )}
