...
import type { GreenhouseProbeResult } from '@/lib/health/probes/greenhouse-api';

const aliveProbe = async (): Promise<ProbeResult> => ({
    finalStatus: 200,
...
describe('checkJobHealth — known-403 host bypass', () => {
    ...
    it('still routes to source-specific probes BEFORE the bypass check (greenhouse beats bypass)', async () => {
        const greenhouse = vi.fn(async (): Promise<GreenhouseProbeResult> => ({
            status: 'alive',
            httpStatus: 200,
        ...
        const result = await checkJobHealth(
            ...
            'greenhouse',
            { greenhouseProbeImpl: greenhouse, probeImpl: probe, externalId: 'greenhouse-talkiatry-12345' },
        );
    ...
    it('handles malformed URLs gracefully (no bypass, falls through to probe)', async () => {
        const probe = vi.fn(aliveProbe);
        await checkJobHealth('not a url at all', null, { probeImpl: probe });
