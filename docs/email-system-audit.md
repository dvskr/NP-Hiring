...
- **Employer diversity cap (added 2026-05-01)** — `sendJobAlerts` now caps any single employer at 2 of the 10 displayed jobs per email. Previously, a company posting many listings (e.g. MindPath Health with jobs in 50 cities) could fill every slot, making the email feel like spam from one company. Algorithm: after the cross-alert dedup + qualityScore sort, walk the list and skip jobs once an employer has 2 entries; if fewer than 10 unique-employer slots are filled (narrow criteria with few matching companies), pull from the per-employer overflow so the full match count is preserved.
