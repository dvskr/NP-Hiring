...
async function main() {
  ...
  // (e.g. a mistakenly-set prod DATABASE_URL/DIRECT_URL) would destroy a
  ...
  // Create jobs one by one and generate slugs
  ...
  await prisma.siteStat.upsert({
    ...
    create: {
      ...
      totalSubscribers: 0,
      totalCompanies: testJobs.length,
    },
  })

  console.log('✓ Database seeded with test jobs (with slugs)')
}
